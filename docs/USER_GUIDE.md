...
在 **2 字幕样式** 选择字体、字号、已唱/未唱颜色、画面横纵位置、同时显示 1–3 句以及预亮/余韵时间。横纵位置以画面宽、高的百分比设定；Y 从上到下计算，89% 位于画面偏下位置。滑块会随字号和行数限制可用纵向范围，防止字幕行被挤出画面。

选择视频后，样式预览默认停在第一帧。拖动预览下方时间轴，可查看视频任意位置的背景画面，样式与歌词改动会即时预览。黄色显示约一半高亮进度，仅用于看位置、行数与颜色；真实高亮时序以渲染结果为准。预览在浏览器本地读取视频，点击生成时才会上传到本地项目服务。浏览器无法解码的容器（如部分 MKV）无法显示画面，可换兼容的视频文件。

历史任务的样式重渲染仍可改位置和行数；若要在原始视频帧上看预览，请重新选择对应的原视频。纯音频任务没有视频帧，会显示示意背景。

日文可尝试“日文整单元高亮”，减少单元内部逐字插值造成的视觉误差；所选字体需已安装并支持歌词文字。
...
生成完成后，结果区会显示 **自动验收**。先看输入和输出行数，再按状态操作：

| 状态 | 含义 | 建议操作 |
| --- | --- | --- |
| 需要处理 | 漏句、时间越界、重叠或无效字词时间等明确问题 | 展开验收提示，核对歌词、修正或补回缺失行 |
| 建议复核 | 有声学风险，或缺少当前版本依据 | 展开逐句依据，试听提示句 |
| 可进入抽查 | 已检查的项目没有触发问题 | 抽听开头、结尾、重复段和长音；不是自动保证准确 |

提示和重试详情默认折叠，避免长歌占满页面。旧记录缺少原始歌词时会显示“完整性尚未核实”，不会把未知当作通过。人工改过的时间不继续沿用旧版声学分数。

**疑难句有限重试** 在 Whisper 与自动转写流程中默认启用，沿用所选模型，每轮最多尝试 3 句、每句最多两路已有声源、窗口不超过 20 秒。相邻句必须有可靠依据；连续风险句会跳过，提示人工确认锚点。不补写缺失歌词、不重算整首，也不保证所有提示都能消除。

已有结果可点 **局部重试疑难句 · 生成新版本**。先提交或撤销草稿修改。候选必须保留完整文本、不越过相邻句、通过字词时间结构检查，并有足够证据改善才会替换。接受修改时生成可回退版本；没有合适候选时保持当前版本，并显示跳过原因。旧记录没有可靠声学依据时可能无法重试。

![可折叠波形与句界编辑](images/alignment-waveform.png)

展开 **人声波形 · 拖动句界与循环试听**：

1. 下拉选择要修改的句子，按需要选择 12 / 24 / 48 秒窗口。
2. 青色竖线为起点、黄色竖线为终点；拖动后同步更新下方微调表的草稿。
3. 点击波形定位；聚焦波形后用左右键每次移动 0.1 秒。
4. 点击 **循环此句**，按草稿起止循环播放；可配合下方试听速度。折叠波形会退出循环。
5. 点击 **应用并重新渲染**，字幕才会使用新时间；之后可切回原始版本比较。

波形优先使用分离人声，否则使用对齐音轨或原始音轨，并标明声源。它表示能量变化，不能判断唱的是哪句歌词。试听仍使用成片音轨；建议先保留原唱完成纠错，再制作去人声成片。首次展开需要提取波形，之后复用缓存；源音频丢失时仍可使用手动时间输入。

要客观比较不同算法或参数，使用独立人工标注的 [对齐评测集](EVALUATION_DATASET.md)。网页验收与评测误差是两种不同依据。
